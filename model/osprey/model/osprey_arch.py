...
from .constant import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class OspreyMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if cur_input_ids.numel() > 0:
                if getattr(self.config, 'tune_mm_mlp_adapter', False) and getattr(self.config, 'mm_use_im_start_end', False):
                    ...
                    mask_idx_ = sum(t.shape[0] for t in cur_new_input_embeds) + mask_idx
                    mask_idx_list.append(mask_idx_)
        ...
        # import pdb;pdb.set_trace()
        # print(new_input_embeds.size())
        pos_idx_list = [idx+1 for idx in mask_idx_list]
        pos_idx_list = torch.cat(pos_idx_list, dim=0)
        mask_idx_list = torch.cat(mask_idx_list, dim=0)
        mask_t = torch.zeros(new_input_embeds.shape[0], new_input_embeds.shape[1], dtype=torch.bool).to(mask_idx_list.device)
        mask_t[torch.arange(mask_t.size(0)).unsqueeze(1), mask_idx_list] = True
        pos_t = torch.zeros(new_input_embeds.shape[0], new_input_embeds.shape[1], dtype=torch.bool).to(mask_idx_list.device)
        pos_t[torch.arange(pos_t.size(0)).unsqueeze(1), pos_idx_list] = True
        # return None, attention_mask, past_key_values, new_input_embeds, new_labels, mask_t, pos_t
