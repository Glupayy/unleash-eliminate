...
def init_cocostuff(dataset_dir):
    ...
    # Annotations
    # print(os.path.join(dataset_dir, "cocostuff", "train2017", "*.png"))
    cocostuff_labels = glob.glob(os.path.join(dataset_dir, "cocostuff", "train2017", "*.png"))
    # print('cocostuff_labels',len(cocostuff_labels))
    ...
    # print('cocostuff_images',len(cocostuff_images))
...
class SemanticSegmDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_dir, tokenizer, global_image_encoder, epoch_samples=500 * 8 * 2 * 10,
        ...
        for ds in self.semantic_seg_ds_list:
            ...
            print(f'\033[92m----SEG-{"Val" if validation else "Train"}: Loaded Semantic_Segm - {ds} dataset ----\033[0m')
    ...
    def __getitem__(self, idx):
        ...
        if dataset_name in ["paco_lvis", "pascal_part"]:
            ...
            # print(self.dataset_dir.replace("Semantic_Segm", ""))
            image_path = os.path.join(self.dataset_dir, dataset_name, "VOCdevkit", "VOC2010", "JPEGImages", file_name) if dataset_name == "pascal_part" else os.path.join(self.dataset_dir.replace("Semantic_Segm", ""),"coco_2017", file_name)
            # print("paco_pascal",image_path)
