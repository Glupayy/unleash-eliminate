...
def combine_coco_captions(annotation_path):
    ...
    all_caps = json.load(open(annotation_path))
    all_caps = {'annotations': all_caps['annotations']}
    return all_caps 
...
def combine_coco_instances(annotation_path):
    ...
    all_caps = json.load(open(annotation_path))
    all_instances = {'annotations': all_caps['annotations']}
...
class CHAIR(object):
    ...
    def __init__(self, coco_path):
        ...
        #read in synonyms， 同义替换！
        synonyms = synonyms_txt.splitlines()
        synonyms = [s.strip().split(', ') for s in synonyms]
        self.mscoco_objects = [] #mscoco objects and *all* synonyms  所有的！！，每个占一个单位
        ...
        # print("mscoco_objects:", self.mscoco_objects)
    ...
    def get_annotations_from_captions(self):
        ...
        for i, annotation in enumerate(caption_annotations):
            ...
            # note here is update, so call get_annotations_from_segments first 从类别，更新为句子！
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):  
        ...
        for i in tqdm.trange(len(caps)):
            ...
            gt_objects = imid_to_objects[imid]  # gt 的主要物体  如['person','person']
            cap_dict = {'image_id': imid, 
                        ...
                        'hallucination_idxs': [], 
                        'words': raw_words   #  如 ['a', 'bush', 'of', 'plant', 'behind', 'middle', 'woman']
                        }
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cap_file", type=str, default='output_referseg_osprey_only_lora_lm_head_e-9.json',
    ...
    parser.add_argument("--cache", type=str, default="chair_new1.pkl",
                        help="pre inited CHAIR evaluator object, for fast loading.")
    parser.add_argument("--coco_path", type=str, default='captions_refcocog_gt.json',
    ...
    if args.save_path:
        save_hallucinated_words(args.save_path, cap_dict['overall_metrics'])
