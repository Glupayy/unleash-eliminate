...
class GenerationMode(ExplicitEnum):
    ...
    CYCD_GENERATION = "cycd_generation"
    # Beam methods
...
class GenerationConfig(PushToHubMixin):
    ...
    def __init__(self, **kwargs):
        ...
        # CYCD generation
        self.cycd_layers = kwargs.pop("cycd_layers", None)
        self.layers_prob = kwargs.pop("layers_prob", None)
    ...
    def get_generation_mode(self, assistant_model: Optional["PreTrainedModel"] = None) -> GenerationMode:
        ...
        if self.cycd_layers is not None:
            if generation_mode in ("greedy_search", "sample"):
                generation_mode = GenerationMode.CYCD_GENERATION
            else:
                raise ValueError(
                    "You've set `cycd_layers`, which triggers CYCD generate. Currently, CYCD generate "
                    "is only supported with Greedy Search and Sample."
                )
