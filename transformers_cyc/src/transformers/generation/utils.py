...
class GenerationMixin:
    ...
    def _get_logits_warper(
        ...
        return warpers
    def _get_logits_warper_temperature_only(
        self,
        generation_config: GenerationConfig,
    ) -> LogitsProcessorList:
        """
    ...
    def generate(
        ...
        **kwargs,
        ...
        elif generation_mode == GenerationMode.CYCD_GENERATION:
            # 11. prepare logits warper
            logits_warper = self._get_logits_warper_temperature_only(generation_config)

            # 13. run sample
            result = self._cycd_decoding(
                input_ids,
                cycd_layers=generation_config.cycd_layers,
                layers_prob=generation_config.layers_prob,
                logits_processor=prepared_logits_processor,
                logits_warper=logits_warper,
                stopping_criteria=prepared_stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
    ...
    def _cycd_decoding(
        ...
        synced_gpus: bool = False,
        streamer: Optional["BaseStreamer"] = None,
        cycd_layers: Optional[List[int]] = None,
        layers_prob: Optional[List[float]] = None,
        **model_kwargs,
    ) -> Union[GenerateNonBeamOutput, torch.LongTensor]:
        ...
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        model_kwargs["cache_position"] = torch.arange(cur_len, device=input_ids.device)
        mature_layer = self.config.num_hidden_layers+1 # + embedding
        candidate_premature_layers = [i for i in cycd_layers if i < mature_layer]
        candidate_layers_prob = None
        if layers_prob is not None:
            candidate_layers_prob = layers_prob
        lm_head = self.get_output_embeddings()

        input_ids_c = input_ids.clone()
        input_ids_c[input_ids_c==-200]=1
        input_ids_c[input_ids_c==32000]=1
        input_ids_c[input_ids_c==32001]=1
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            # prepare model inputs
            ...
            next_token_logits = _cycd_select_contrast(
                candidate_premature_layers, candidate_premature_logits, final_logits , candidate_layers_prob  # inference要改
            )
            

            
            next_token_scores = logits_processor(input_ids_c, next_token_logits)
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            input_ids_c = torch.cat([input_ids_c, next_tokens[:, None]], dim=-1) # for process logits
            if streamer is not None:
                streamer.put(next_tokens.cpu())
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs,
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # import pdb;pdb.set_trace()
            next_token_logits = outputs.logits[:, -1, :]
            # print('logit processor',logits_processor)
            # print('logit warper', logits_warper)
...
def _cycd_select_contrast(
    ...
    if len(candidate_premature_layers) == 1:
        ...
        if candidate_layers_prob is not None:
            p_js = F.softmax(js_divs, dim=0)
            p_js = p_js+candidate_layers_prob
            p_js = F.softmax(p_js, dim=0)
            best_idx = torch.multinomial(p_js, num_samples=1).squeeze()
            premature_layer = candidate_premature_layers[int(best_idx.cpu().item())]
        else:
...
def _relative_top_filter(
    ...
    relative_top: float = 0.1, #0.1 is org
